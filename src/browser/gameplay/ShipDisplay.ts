...
import { Ship, HYPERSPACE_DELAY, ShipDestroyOptions, ShipObserver } from "../../core";
import { Explosion, ShipSpawnAnimation } from "../animations";
import { GameTheme } from "../GameTheme";
import { PowerupFilter } from "../filters";

export const createShipGeometry = (color: number, lineWidth = 3, height = 60): Graphics => {
...
export const createShipFireGeometry = (color: number): Graphics => {
...
export class ShipDisplay extends Container implements ShipObserver, Tickable {
    ...
    constructor(ship: Ship, theme: GameTheme) {
        ...
        this._shipGraphics = createShipGeometry(theme.foregroundColor);
        this._fireGraphics = createShipFireGeometry(theme.fireColor);
    ...
    onDestroy({ hit }: ShipDestroyOptions): void {
        if (hit && this.parent) {
