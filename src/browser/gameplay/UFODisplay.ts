...
import { UFOObserver, UFO, UFOType, UFO_SIZES, UFODestroyOptions } from "../../core";
...
export const createUFOGeometry = (lineWidth: number, scale: number, color: number): Graphics => {
...
export class UFODisplay extends Container implements UFOObserver {
    ...
    constructor(ufo: UFO, theme: GameTheme) {
        ...
        const sprite = createUFOGeometry(LINE_WIDTHS[ufo.type], UFO_SIZES[ufo.type], theme.ufoColor);
    ...
    onDestroy({ hit }: UFODestroyOptions): void {
        if (hit && this.parent) {
